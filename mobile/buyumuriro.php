...
$messagecontent="umukiriya aguhaye amafaranga:".$amount."arava Kuri Konti Yawe:".$senderAccount."Ajya kuri Konti ye";


$messagreceiver="ubikije amafaranga:".$amount."kuri Konti yawe:".$accoudestination."Murakoaze";
...
if ($result1->num_rows > 0) {
    ...
    $newReceiverBalance=$balReceiver-$recharge-$amount;
...
if($conn->query($sql5)===TRUE && $conn->query($sql6)===TRUE && $conn->query($sql4)===TRUE) {
   ...
   if($conn->query($sql2) === TRUE && $conn->query($sql3)===TRUE && $conn->query($sql1)===TRUE){
      ...
      $arr = array('error'=>" no error",'result'=>"success","message"=>"Umuriro wawuguze Neza,urakoze");
   ...
   $arr = array('error'=>"error",'result'=>"failed","message"=>"Kugura umuriro Ntibikunze,Mwongere mugerageze");
  ...
  $arr = array('error'=>"error",'result'=>"failed","message"=>"konti ya Reg ntibaho");
